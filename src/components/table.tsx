...
export const Table = <T,>(props:TableProps<T>) => {
    ...
    const headers = Object.keys(props.items[0]!)
                          .map(h => <th key={h} className="w-1/2 border capitalize border-slate-300 dark:border-slate-600 font-semibold p-4 text-slate-900 dark:text-slate-200 text-left">{h}</th>)
